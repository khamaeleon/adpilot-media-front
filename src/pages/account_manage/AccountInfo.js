...
function AccountInfo(){
    ...
    if(tokenResultInfo.role !== 'NORMAL') { // 어드민 계정
      if(adminInfoState.convertedUser !== ''){
        accountUserProfile(adminInfoState.convertedUser).then(response => { //정산 프로필 조회
          ...
          let userName = response !== null ? adminInfoState.convertedUser : null;
  ...
  const handleAdminApi = (userName) => { // 어드민 정산 수익, 월별 수익 현황 조회 API
    userName !== null ? accountMonthlyListTableData(userName).then(response => { // 월별 수익
      response !== null && setAccountInfoTableData(response)
    }) : setAccountInfoTableData([])
  ...
  const handleSearchResult = (accountUserData) => {
    ...
    accountUserProfile(accountUserData.username).then(response => {
      ...
      setAccountProfileState(response);
      let userName = response !== null ? accountUserData.username : null;
      handleAdminApi(userName)
    })
  }
  ...
  return(
    <>
      <Navigator/>
      {
        adminInfoState.convertedUser !== '' && <RowSpan style={{justifyContent: 'flex-end', marginTop: 0}}><SearchUser title={'매체 계정 전환'} onSubmit={handleSearchResult} btnStyle={'AccountButton'}/></RowSpan>
      }
