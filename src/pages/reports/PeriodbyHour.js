...
import {
  reportsStatics,
  reportsStaticsAll,
  reportsStaticsHourlyAllColumn,
  reportsUserStaticsHourlyAllColumn
} from "./entity/period";
import {
  Board,
  BoardHeader,
...
import {
  selectAdminStaticsHourAll,
  selectUserStaticsHourlyAll
} from "../../services/reports/periodAxios";
import {UserInfo} from "../layout";

/** 일자별 차트 **/
function MyResponsiveHourBar(props) {
...
/** 기간별 보고서 **/
export default function ReportsHourPeriod(){
  ...
  const handleSearchCondition = async({skip,limit,sortInfo}) => {
    ...
    const condition = {
      ...
      sortType: sort('HOUR_DESC',sortInfo)
    }

    if(tokenInfoState.role === 'NORMAL') {// 일반유저
      result = await selectUserStaticsHourlyAll(tokenInfoState.id, condition).then(response => {
      if(response !== null) {
      ...
      result = await selectAdminStaticsHourAll(creativeInfo.id, condition).then(response => {
        if(response !== null) {
          const data = response.content;
          setTotalCount(response.totalElements)
          return {data, count: response.totalElements}
  ...
  const handleChartSearchCondition = async() => {
    ...
    if(tokenInfoState.role === 'NORMAL') {// 일반유저
      return await selectUserStaticsHourlyAll(tokenInfoState.id, condition).then(response => {
        if(response !== null) {
          const data = response.content;
          data.map((item,key) => {
      ...
      return await selectAdminStaticsHourAll(creativeInfo.id,condition).then(response => { // 어드민 유저
        if(response !== null) {
          const data = response.content;
          data.map((item, key) => {
            Object.assign(data[key], {clickRate: item.exposureCount !== 0 ? (item.validClickCount / item.exposureCount) * 100 : 0})
  ...
  return(
    <Board>
      ...
      <BoardSearchResult>
        <Table columns={userInfoState.email === '' ? reportsStaticsHourlyAllColumn : reportsUserStaticsHourlyAllColumn}
