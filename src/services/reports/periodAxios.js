...
export async function selectUserStaticsHourlyAll(userId, params) {
  let returnVal = null
  await MediaAxios('POST', `/statistics/${userId}/all/hourly`, params)
  .then(response => {
    const { data, statusCode } = response;
    if(statusCode === 200){
      returnVal = data;
...
export async function selectAdminStaticsHourAll(userId, params) {
  let returnVal = null

  if(userId !== undefined) {
    await AdminAxios('POST', `/media/statistics/${userId}/all/hourly`, params)
    .then(response => {
      const { data, statusCode } = response;
      if(statusCode === 200){
        returnVal = data;
    ...
    await AdminAxios('POST', `/media/statistics/all/hourly`, params)
    .then(response => {
      const { data, statusCode } = response;
      if(statusCode === 200){
        returnVal = data;
